...
try:
    ...
    logger.warning("⚠️ sentence-transformers not available, embeddings will be limited")
...
class KnowledgeBaseIngester:
    ...
    def extract_document_info(self, data: Dict[str, Any], source_type: str) -> Dict[str, Any]:
        ...
        if source_type == 'github':
            ...
            doc_info.update({
                'title': data.get('title', 'Web Document'),
    ...
    async def test_search(self):
        ...
        if not self.embedding_model:
            logger.warning("⚠️ Cannot test search without embedding model")
            return
        ...
        for query in test_queries:
            try:
                ...
                # Search ChromaDB
                results = await self.vector_store.search_similar(query_embedding, limit=3)
                logger.info(f"🔍 Query: '{query}' → {len(results)} results")
                if results:
                    top_result = results[0]
                    logger.info(f"   📄 Top result: {top_result['metadata'].get('title', 'Unknown')[:50]}...")
...
async def main():
    ...
    try:
        ...
        print(f"Vector dimension: {result['database_stats'].get('vector_dimension', 'N/A')}")
        print("="*60)
