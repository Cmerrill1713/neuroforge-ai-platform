...
async def start_optimization(config: OptimizationConfig) -> Dict[str, Any]:
    ...
    try:
        ...
        # Handle both old format (list) and new format (dict with examples)
        if isinstance(data, list):
            # If it's a list, check if first item has 'examples' key
            if len(data) > 0 and isinstance(data[0], dict) and 'examples' in data[0]:
                golden_examples = data[0]['examples']
            else:
                golden_examples = data
        elif isinstance(data, dict) and 'examples' in data:
            golden_examples = data['examples']
        else:
            raise HTTPException(status_code=500, detail="Invalid golden dataset format")
        
        # Convert examples to expected format
        converted_examples = []
        for ex in golden_examples:
            converted = {
                'query': ex['query'],  # Keep original key name
                'context': ex['context'],
                'rubric': 'Provide a helpful, accurate response.',
                'expected_output': ex['expected_output']
            }
            converted_examples.append(converted)
        
        golden_examples = converted_examples
        
        # Run evolution with timeout
        import asyncio
        try:
            best_genome = await asyncio.wait_for(
                evolutionary_integration.optimize_comprehensive(
            ...
            raise HTTPException(status_code=408, detail="Evolution timed out after 60 seconds")
