...
class NL2Prompt(dspy.Module):
    ...
    def __init__(self, schema=PromptSpec):
        super().__init__()
        # Create signature string (DSPy 2.x syntax)
        self.predict = dspy.Predict(
            "user_query, context, rubric -> spec",
            instructions="Output ONLY a JSON matching PromptSpec. No prose, no markdown."
        )
        self.schema = schema
    
    def forward(self, user_query: str, context: str, rubric: str) -> PromptSpec:
        """Generate and validate prompt spec"""
        raw = self.predict(user_query=user_query, context=context, rubric=rubric).spec
        spec = dspy.parse_as(self.schema, raw)  # Schema validation
        return spec
